...
if __name__ == "__main__":
    ...
    SOFT_RECOVERY_SQL: str = (
        """UPDATE users SET deleted_at = null, deleted = false WHERE id IN ({users_to_restore})"""
    )
    RTBF_RECOVERY_SQL: str = (
        """UPDATE {tablename} SET rtfb_hidden = false WHERE author_id IN ({users_to_restore})"""
    )
    ...
    with open(
        ...
        rtbf_hidden_resources: tuple[str] = tuple(
            configData["rtbf_hidden_resources"]
        )  # Tuple of names of tables which are hidden by RTBF
        ...
        with CONNECTION.cursor() as CURSOR:
            while True:
                ...
                for querydata in _streamd_queries:
                    ...
                    userID: str = querydata[1].pop(
                        "id"
                    )  # Keep as str, otherwise we'll need to int user ids cast back to str for str.join()
                ...
                try:
                    ...
                    CURSOR.execute(
                        SOFT_RECOVERY_SQL.format(",".join(rtbf_data + non_rtbf_data))
                    )
                    ...
                    for table in rtbf_hidden_resources:
                        CURSOR.execute(
                            RTBF_RECOVERY_SQL.format(
                                tablename=table, users_to_restore=",".join(rtbf_data)
                            )
                        )
