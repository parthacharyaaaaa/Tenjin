...
def main() -> None:
    ...
    UPDATION_SQL: str = (
        "UPDATE {table_name} SET {column_name} = %(counter)s WHERE id = %(id)s;"
    )
    ...
    with open(
        ...
        counter_metadata: dict[str, list[str]] = json.loads(config_file.read()).get(
            "counter_metadata"
        )
        assert (
            counter_metadata
        ), "Config file for batch workers missing mandatory field: counter_metadata"
    ...
    with CONNECTION.cursor() as CURSOR:
        while True:
            for counter_mapping in counter_hashmap_names:
                ...
                counter_data: list[dict[str, int]] = [
                    {"id": key, "counter": counter} for key, counter in _res[0].items()
                ]  # argslist argument
                try:
                    execute_batch(
                        cur=CURSOR,
