...
if __name__ == "__main__":
    ...
    INSERTION_SQL: str = (
        "INSERT INTO {table_name} {tColumns} VALUES %s ON CONFLICT DO NOTHING;"
    )
    with CONNECTION.cursor() as dbCursor:
        while True:
            ...
            backoffIndex = (
                0  # Reset backoff index upon succesfull network request cycle
            )
            ...
            for query_data in _streamd_queries:
                try:
                    ...
                    table: str = table_data.pop(
                        "table"
                    )  # Remove and read helper field 'table'
                    if table not in dtypes_cache:
                        dTypesList: list[type] = getDtypes(
                            dbCursor, table, includePrimaryKey=True
                        )  # Discriminators for weak entities are provided in streams as they are PKs for assosciated strong entities
                        pk_dtypes: int = len(dTypesList) - len(
                            getDtypes(dbCursor, table, includePrimaryKey=False)
                        )
                    ...
                    flag_names_mapping[table].append(
                        flag_name_template.format(
                            resource=table,
                            identifiers=":".join(
                                list(table_data.values())[: pk_cache[table]]
                            ),
                        )
                    )
            ...
            for table, qargs in query_groups.items():
                ...
                try:
                    ...
                    print(
                        f"[{ID}]: Error in executing batch insert for table {table}, exception: {pg_error.__class__.__name__}"
                    )
