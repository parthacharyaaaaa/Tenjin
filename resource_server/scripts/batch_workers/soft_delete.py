...
if __name__ == "__main__":
    ...
    DEPENDENT_MAPPING: dict[str, dict[str, str]] = {
        "forums": {"posts": "parent_forum"},
        "posts": {"comments": "parent_post"},
    }
    # Initialize updation SQL
    UPDATION_SQL: str = (
        "UPDATE {tablename} SET time_deleted = %s, deleted = true WHERE id IN ({ids_to_delete});"
    )
        ...
        with CONNECTION.cursor() as CURSOR:
            while True:
                ...
                table_groups: dict[str, list[list[int], list[datetime]]] = (
                    {}
                )  # {<tablename> : [<ordered list of IDs to delete>, <ordered list of datetime objects for 'time_deleted>' column]}
                for queryData in _streamd_queries:
                    ...
                    try:
                        ...
                        print(
                            f"[{ID}]: Received invalid query params from entry: {queryData[0]}"
                        )
                        print(format_exc())
                ...
                for table, targetList in table_groups.items():
                    ...
                    try:
                        execute_batch(
                            CURSOR,
                            UPDATION_SQL.format(
                                tablename=table, ids_to_delete=table_groups[table][0]
                            ),
                        ...
                        if table in DEPENDENT_MAPPING:
                            for dependent_table, dependent_fk in DEPENDENT_MAPPING[
                                table
                            ].items():
                                enqueue_cascade_soft_deletes(
                                    CURSOR,
                                    interface,
                                    target_table=dependent_table,
                                    fk_colname=dependent_fk,
                                    parent_pk_seq=table_groups[table][0],
                        ...
                        print(
                            f"[{ID}]: Traceback: {e.__class__.__name__}\n{format_exc()}"
                        )
                        exit(500)
                        ...
                        print(
                            f"[{ID}]: Error in executing batch delete for table {table}, exception: {pg_error.__class__.__name__}"
                        )
