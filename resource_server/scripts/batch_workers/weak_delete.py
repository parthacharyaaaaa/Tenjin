    ...
    with CONNECTION.cursor() as dbCursor:
        while True:
            ...
            for table, qargs in query_groups.items():
                pk_cols_template = " AND ".join(
                    f"{k} = %({k})s" for k in qargs[0].keys()
                )
                try:
                    execute_batch(
                        cur=dbCursor,
                        sql=DELETION_SQL.format(
                            table_name=table, pk_columns_template=pk_cols_template
                        ),
                        argslist=qargs,
                        page_size=batchSize,
                    )
