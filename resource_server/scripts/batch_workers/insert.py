...
if __name__ == "__main__":
    ...
    dtypes_cache: dict = {}
    tables_cache: dict[str, dict[str, str]] = (
        {}
    )  # Keys are table names, values are dictionaries with templates, and typed columns

    # Initialize mapping to group insertion queries by tables
    query_groups: dict[str, list[dict[str, Any]]] = (
        {}
    )  # dict[<tablename> : argslist[dict[<attribute> : <value>]]]
    ...
    with CONNECTION.cursor() as dbCursor:
        while True:
            try:
                _streamd_queries: list[tuple[str, dict[str, str]]] = interface.xrange(
                    streamName, count=batchSize
                )
            ...
            for query_data in _streamd_queries:
                try:
                    table_data = {
                        k: None if v == "" else v for k, v in query_data[1].items()
                    }
                    table: str = table_data.pop(
                        "table"
                    )  # Remove helper field 'table' from table_data
                    ...
                    if table not in dtypes_cache:
                        ...
                        dtypes_cache[table] = [
                            MAPPED_DTYPES.get(dtype, str)
                            for dtype in get_column_types(dbCursor, table)
                        ]

                    # Finally cast to Python data types compatible with psycopg2
                    table_data = {
                        k: dtypes_cache[table][idx](v) if v else v
                        for idx, (k, v) in enumerate(table_data.items())
                    }
                    ...
                    print(
                        f"[{ID}]: Received invalid query params from entry: {query_data[0]}"
                    )
                ...
                try:
                    ...
                    if cached_data:
                        ...
                        template: str = (
                            ...
                            + ", ".join(
                                f"%({k})s"
                                for k in (*columns, "_python_enumeration_index")
                            )
                            + ")"
                        )  # Inject enumeration index into the VALUES list

                        # Prepare typed column names for 2nd CTE
                        column_types: list[str] = get_column_types(dbCursor, table)
                        typed_columns = ",".join(
                            f"{col}::{column_types[idx]}"
                            for idx, col in enumerate(columns)
                        )

                        # Prepare join clause for final result
                        join_clauses: str = " AND ".join(
                            f"""inserted_query_data.{col} = input_query_data.{col}::{column_types[idx]}
                                                         OR (inserted_query_data.{col} IS NULL AND input_query_data.{col}::{column_types[idx]} IS NULL)"""
                            for idx, col in enumerate(columns)
                        )  # Since NULL <> NULL in SQL, brilliant >:/
                        tables_cache[table] = {
                    ...
                    _res: tuple[tuple[int, int]] = execute_values(
                        cur=dbCursor,
                        sql=INSERTION_SQL.format(
                            table_name=table,
                            table_columns=tColumns,
                            typed_table_columns=typed_columns,
                            join_clauses=join_clauses,
                        ),
                        argslist=[
                            qarg | {"_python_enumeration_index": qidx}
                            for qidx, qarg in enumerate(qargs)
                        ],
                        template=template,
                        page_size=len(query_groups),
                        fetch=True,
                    )
                    CONNECTION.commit()
                    if not _res:
                    ...
                    cache_entries: dict[str, dict[str, Any]] = {
                        f"{table}:{returned_values[0]}": {"id": returned_values[0]}
                        | rediserialize(qargs[returned_values[1]])
                        for returned_values in _res
                    }
                    ...
                    print(
                        f"[{ID}]: Error in executing batch isnert for table {table}, exception: {pg_error.__class__.__name__}"
                    )
