...
if __name__ == "__main__":
    ...
    AUDIT_THRESHOLD: datetime = CURRENT_DAY - timedelta(
        days=int(os.environ["AUDIT_THRESHOLD"])
    )
    USER_AUDIT_THRESHOLD: datetime = CURRENT_DAY - timedelta(
        days=int(os.environ["ACCOUNT_RECOVERY_PERIOD"])
    )
        ...
        with CONNECTION.cursor() as dbCursor:
            ...
            for idx, table in enumerate(("posts", "comments")):
                ...
                try:
                    ...
                    dbCursor.execute(
                        DELETION_SQL.format(
                            table=table, placeholder=placeholder_string
                        ),
                        vars=(*_res,),
                    )
                    CONNECTION.commit()
                    ...
                    print(
                        f"[HARD DELETION JOB]: Exception encountered for table {table}. Details:\n{format_exc()}"
                    )
            ...
            dbCursor.execute(
                """
                             ...
                             """.format(placeholder=placeholder_string),
                vars=(_res,),
            )
            CONNECTION.commit()
            ...
            _res: tuple[tuple[int, int]] = dbCursor.fetchall()
            _res: tuple[tuple[str, str]] = tuple(
                map(lambda x: (str(x[0]), str(x[1])), _res)
            )
            ...
            if _res:
                ...
                dbCursor.execute(
                    "DELETE FROM forum_admins WHERE (user_id, forum_id) IN ({placeholder});".format(
                        placeholder_string
                    ),
                    vars=(*_flattend_res,),
                )
            ...
            with interface.pipeline() as pipe:
                for forumID, delta in forum_decrement_mapping.items():
                    ...
                    pipe.hsetnx(
                        hashmap_name, forumID, counter_name
                    )  # Set new entry in forum:admins if not present
            ...
            if _res:
                ...
                dbCursor.execute(
                    "DELETE FROM users WHERE id IN ({})".format(placeholder_string),
                    vars=(*_res,),
                )
