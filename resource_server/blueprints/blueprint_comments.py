...
def comment_on_post() -> tuple[Response, int]:
    try:
        ...
        raise BadRequest(
            "To post a comment, please ensure that the post ID and a valid comment body is provided"
        )
    ...
    if (
        post_mapping.get("closed") == "1"
    ):  # Cached resources have been serialized and hence booleans are casted to 0/1 str
    ...
    comment: Comment = Comment(
        authorID=g.DECODED_TOKEN["sid"],
        parentForum=post.forum_id,
        epoch=datetime.now(),
        body=body.strip(),
    ...
    RedisInterface.xadd(
        "INSERTIONS",
        rediserialize(comment.__attrdict__()) | {"table": Comment.__tablename__},
    )
    return (
        jsonify(
            {
                ...
                "body": body,
                "author": g.DECODED_TOKEN["sub"],
                "time": comment.time_created.isoformat(),
            }
        ),
        202,
    )
...
def delete_comment(comment_id: int) -> tuple[Response, int]:
    ...
    comment_mapping: dict[str, Any] = resource_existence_cache_precheck(
        client=RedisInterface,
        identifier=comment_id,
        resource_name=Comment.__tablename__,
        cache_key=cache_key,
        deletion_flag_key=flag_key,
    )
    ...
    try:
        if not comment_mapping:
            ...
            comment: Comment = db.session.execute(
                select(Comment).where(
                    (Comment.id == comment_id) & (Comment.deleted.is_(False))
                )  # Allow explicit deletion of RTBF hidden comments too
            ).scalar_one_or_none()
            if not comment:
        ...
        if int(comment_mapping.get("author_id")) != g.DECODED_TOKEN.get("sid"):
            ...
            forumAdmin: ForumAdmin = db.session.execute(
                select(ForumAdmin).where(
                    (ForumAdmin.forum_id == int(comment_mapping.get("parent_forum")))
    ...
    try:
        ...
        update_global_counter(
            ...
            table=Post.__tablename__,
            column="total_comments",
            identifier=int(comment_mapping["parent_post"]),
        )
        update_global_counter(
            interface=RedisInterface,
            delta=-1,
        ...
        with RedisInterface.pipeline(transaction=False) as pipe:
            ...
            pipe.xadd(
                "SOFT_DELETIONS", {"id": comment_id, "table": Comment.__tablename__}
            )
            pipe.hset(
                cache_key,
...
def vote_comment(comment_id: int) -> tuple[Response, int]:
    ...
    comment_mapping, latest_intent, deletion_intent = resource_cache_precheck(
        RedisInterface,
        comment_id,
        cache_key,
        f"delete:{cache_key}",
    ...
    if (
        lock or latest_intent == incoming_intent
    ):  # Race condition, or latest intent is same as the intent carried by this request. Reject
        raise Conflict(f"A request for this action is currently enqueued")
    ...
    try:
        if not (comment_mapping or latest_intent):
            ...
            joined_result: Row = db.session.execute(
                select(Comment, CommentVote.vote)
                .outerjoin(
                    CommentVote,
                    (CommentVote.voter_id == g.DECODED_TOKEN["sid"])
    ...
    try:
        ...
        update_global_counter(
            ...
            table=User.__tablename__,
            column="aura",
            identifier=comment_mapping.get("author_id"),
        )
        ...
        with RedisInterface.pipeline(transaction=False) as pipe:
            pipe.xadd(
                ...
                fields={
                    ...
                    "vote": vote,
                    "table": CommentVote.__tablename__,
                },
            )
...
def unvote_comment(comment_id: int) -> tuple[Response, int]:
    ...
    comment_mapping, latest_intent, pending_deletion = resource_cache_precheck(
        RedisInterface,
        comment_id,
        cache_key,
        f"delete:{cache_key}",
        ...
        lock_key,
        Comment.__tablename__,
        conflicting_intent=RedisConfig.RESOURCE_DELETION_PENDING_FLAG,
        allow_deletion=True,
    )
    ...
    response_body: dict[str, str] = {
        ...
        "previous_vote": "downvote" if delta == 1 else "upvote",
    }
    if pending_deletion:
        response_body["deletion_notice"] = (
            f"Comment {comment_id} will be deleted soon, and you may not be able to cast votes on this comment henceforth"
        )
...
def report_comment(comment_id: int) -> tuple[Response, int]:
    ...
    cache_key: str = f"{Comment.__tablename__}:{comment_id}"
    flag_key: str = (
        f'{CommentReport.__tablename__}:{g.DECODED_TOKEN["sid"]}:{report_tag}'  # Reserved for this report tag only, existence would hence imply duplication
    )
    lock_key: str = f"lock:{flag_key}"
    ...
    try:
        if not comment_mapping:
            ...
            prior_report: CommentReport = db.session.execute(
    ...
    if prior_report:  # Post already reported for this reason
        ...
        conflict: Conflict = Conflict(
            "You have already reported this post for this reason"
        )
    ...
    report_mapping: dict[str, str | int] = {
        "user_id": g.DECODED_TOKEN["sid"],
        "comment_id": comment_id,
        "report_tag": report_tag,
        "report_time": epoch_string,
        "report_description": report_desc,
        "table": CommentReport.__tablename__,
    }
