...
RANDOMIZER_SQL = (
    ...
    .where(
        Anime.id
        >= func.floor(func.random() * select(func.max(Anime.id)).scalar_subquery())
    )
    .order_by(Anime.id)
    .limit(50)
)
...
def get_anime(anime_id: int) -> tuple[Response, int]:
    ...
    animeMapping: dict = consult_cache(
        RedisInterface,
        cacheKey,
        RedisConfig.TTL_CAP,
    ...
    # 2: Fallback to DB
    try:
        anime: Anime = db.session.execute(
            select(Anime).where(Anime.id == anime_id)
        ).scalar_one_or_none()
        if not anime:
            RedisInterface.set(
                cacheKey, RedisConfig.NF_SENTINEL_KEY, RedisConfig.TTL_EPHEMERAL
            )  # Announce non-existence
            raise NotFound(f"No anime with id {anime_id} could be found")
    ...
    animeMapping = rediserialize(anime.__json_like__()) | {
        "genres": genres,
        "stream_links": {link.website: link.url for link in streamLinks},
    }
...
def get_random_anime():
    ...
    try:
        ...
        if not found:
            anime: Anime = db.session.execute(
                select(Anime).limit(1).order_by(Anime.id.asc())
            ).scalar_one_or_none()
...
def sub_anime(anime_id: int) -> tuple[Response, int]:
    ...
    if (
        RedisInterface.hget(cacheKey, RedisConfig.NF_SENTINEL_KEY)
        == RedisConfig.NF_SENTINEL_VALUE
    ):
        hset_with_ttl(
            RedisInterface,
            cacheKey,
    ...
    try:
        _res: Row = db.session.execute(
            select(Anime, AnimeSubscription)
            .outerjoin(
                AnimeSubscription,
                (AnimeSubscription.anime_id == anime_id)
                & (AnimeSubscription.user_id == g.DECODED_TOKEN["sid"]),
            )
    ...
    RedisInterface.xadd(
        "WEAK_INSERTIONS",
        {
            ...
            "anime_id": anime_id,
            "table": AnimeSubscription.__tablename__,
        },
    )
...
def get_animes() -> tuple[Response, int]:
    try:
        ...
        raise BadRequest("Failed to load more posts. Please refresh this page")
    pagination_cache_key: str = (
        f'{Anime.__tablename__}:{cursor}:{genreID or "N/A"}:{searchParam or "N/A"}'
    )
    ...
    if animes and all(animes):
        ...
        animes: list[str, Any] = [
            pyserialize(anime, deserialize_mapping=Anime.deserialization_mapping())
            for anime in animes
        ]
        promote_group_ttl(
            RedisInterface,
    ...
    try:
        ...
        if searchParam:
            whereClause.append(
                Anime.title.ilike(f"%{searchParam}%")
            )  # searchParam is a string anyways, so we can safely inject it in an expression >:3
        ...
        rows = db.session.execute(
            select(AnimeGenre.anime_id, Genre._name)
            .join(Genre, Genre.id == AnimeGenre.genre_id)
            .where(AnimeGenre.anime_id.in_(animeIDs))
        ).all()
    ...
    cache_grouped_resource(
        ...
        Anime.__tablename__,
        {result["id"]: rediserialize(result) for result in jsonified_result},
        RedisConfig.TTL_WEAK,
        RedisConfig.TTL_STRONG,
        next_cursor,
        end,
    )
    ...
    counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
        client=RedisInterface,
        hashmaps=[f"{Anime.__tablename__}:{attr}" for attr in counter_attrs],
        identifiers=[anime["id"] for anime in jsonified_result],
    )
...
def get_anime_links(anime_id: int) -> tuple[Response, int]:
    try:
        streamLinks: dict[str, str] = dict(
            db.session.execute(
                select(StreamLink.website, StreamLink.url).where(
                    StreamLink.anime_id == anime_id
                )
            ).all()
        )
    except SQLAlchemyError:
...
def get_anime_forums(anime_id: int) -> tuple[Response, int]:
    ...
    if not anime_mapping:
        ...
        try:
            ...
            hset_with_ttl(
                RedisInterface, cache_key, anime_mapping, RedisConfig.TTL_WEAK
            )
        except SQLAlchemyError:
            genericDBFetchException()
    ...
    if forums and all(forums):
        ...
        forums: list[dict[str, Any]] = [
            pyserialize(forum, deserialize_mapping=Forum.deserialization_mapping())
            for forum in forums
        ]
        return jsonify({"forums": forums, "cursor": next_cursor, "end": end}), 200
    # Cache miss
    try:
        where_clause: BinaryExpression = (Forum.anime == anime_id) & (
            Forum.deleted.is_(False)
        )
        if cursor:
            where_clause &= Forum.id > cursor
        forum_res: list[Forum] = (
            db.session.execute(
                select(Forum)
                .where(where_clause)
                .limit(6)
    ...
    cache_grouped_resource(
        RedisInterface,
        group_key=pagination_cache_key,
        resource_type=Forum.__tablename__,
        resources={
