...
def create_post() -> tuple[Response, int]:
    if not (
        g.REQUEST_JSON.get("forum")
        and g.REQUEST_JSON.get("title")
        and g.REQUEST_JSON.get("body")
    ):
        raise BadRequest(
            "Invalid details for creating a post. Please include: forum id, post title, and post body"
        )
    ...
    try:
        ...
        raise BadRequest(
            "Malformed post details. Title and body must be strings, and forum must be an integer/numeric string"
        )

    # Ensure author and forum actually exist
    author: User = db.session.execute(
        select(User).where(
            (User.id == g.DECODED_TOKEN["sid"]) & (User.deleted.isnot(True))
        )
    ).scalar_one_or_none()
    if not author:
        ...
        nf.__setattr__(
            "kwargs",
            {
                "help": "If you believe that this is an erorr, please contact support",
                "_links": {"login": {"href": url_for("misc.issue_ticket")}},
            },
        )
    forum: Forum = db.session.execute(
        select(Forum).where(Forum.id == forumID)
    ).scalar_one_or_none()
    if not forum:
    ...
    RedisInterface.xadd(
        "INSERTIONS", rediserialize(post.__attrdict__()) | {"table": Post.__tablename__}
    )
    ...
    update_global_counter(
        ...
        column="total_posts",
        identifier=g.DECODED_TOKEN["sid"],
    )
    update_global_counter(
        interface=RedisInterface,
        delta=1,
...
def get_post(post_id: int) -> tuple[Response, int]:
    ...
    post_mapping: Optional[dict[str, Any]] = consult_cache(
        RedisInterface,
        cacheKey,
        RedisConfig.TTL_CAP,
        RedisConfig.TTL_PROMOTION,
        RedisConfig.TTL_EPHEMERAL,
    )

    try:
        resultSet: Row = db.session.execute(
            select(Post, User.deleted)
            .join(Post, Post.author_id == User.id)
            .where(
                (Post.id == post_id)
                & (Post.deleted.is_(False) & (Post.rtbf_hidden.isnot(True)))
            )
        ).first()
        if not resultSet:
            hset_with_ttl(
    ...
    if fetch_relation:
        ...
        # 1: Consult cache
        with RedisInterface.pipeline(
            transaction=False
        ) as pp:  # Get operation need not be constrained by atomicity
        ...
        # 2: Fall back to db
        if not postSaved:
            postSaved = db.session.execute(
                select(PostSave).where(
                    (PostSave.post_id == post_id)
                    & (PostSave.user_id == g.REQUESTING_USER.get("sid"))
                )
            ).scalar_one_or_none()
        if not postVoted:
            postVoted = db.session.execute(
                select(PostVote.vote).where(
                    (PostVote.post_id == post_id)
                    & (PostVote.voter_id == g.REQUESTING_USER.get("sid"))
                )
            ).scalar_one_or_none()
        ...
        with RedisInterface.pipeline(
            transaction=False
        ) as pp:  # Ephemeral set operation need not be constrained by atomicity
            pp.set(
                f'saves:{post_id}:{g.REQUESTING_USER["sid"]}',
                1 if postSaved else 0,
                ex=RedisConfig.TTL_EPHEMERAL,
            )
            pp.set(
                f'votes:{post_id}:{g.REQUESTING_USER["sid"]}',
                -1 if not postVoted else int(postVoted),
                ex=RedisConfig.TTL_EPHEMERAL,
            )
            pp.execute()
...
def edit_post(post_id: int) -> tuple[Response, int]:
    if not (
        g.REQUEST_JSON.get("title")
        or g.REQUEST_JSON.get("body")
        or g.REQUEST_JSON.get("closed")
    ):
        raise BadRequest("No changes sent")

    cache_key: str = f"{Post.__tablename__}:{post_id}"
    post_mapping: dict[str, Any] = resource_existence_cache_precheck(
        client=RedisInterface,
        identifier=post_id,
        resource_name=Post.__tablename__,
        cache_key=cache_key,
    ...
    if post_mapping and int(post_mapping["author_id"]) != g.DECODED_TOKEN["sid"]:
        raise Forbidden(
            "You do not have the permission to edit this post as you are not its owner"
        )
    ...
    if not post_mapping:
        try:
            ...
            if post.author_id != g.DECODED_TOKEN["sid"]:
                raise Forbidden(
                    "You do not have the rights to edit this post as you are not its author"
                )
        except SQLAlchemyError:
            genericDBFetchException()
    ...
    if not update_kw:
        ...
        raise badReq
    try:
        updatedPost: Post = db.session.execute(
            update(Post).where(Post.id == post_id).values(**update_kw).returning(Post)
        ).scalar_one()
        db.session.commit()
    except SQLAlchemyError:
        db.session.rollback()
        raise InternalServerError(
            "An error occured when editing your post, please try again later"
        )
    ...
    hset_with_ttl(
        RedisInterface, cache_key, rediserialize(post_mapping), RedisConfig.TTL_WEAK
    )

    return jsonify({"message": "Post edited.", "post": post_mapping}), 200
...
def delete_post(post_id: int) -> Response:
    cache_key: str = f"{Post.__tablename__}:{post_id}"
    flag_key: str = (
        f"delete:{cache_key}"  # User ID not included in intent flag, because multiple users (owner, admin) can delete a post
    )
    ...
    if not post_mapping:
        try:
            post: Post = db.session.execute(
                ...
                )  # This endpoint still allows RTBF hidden posts to be explicitly deleted
            ).scalar_one_or_none()
            if not post:
                hset_with_ttl(
                    RedisInterface,
    ...
    if int(post_mapping["author_id"]) != g.DECODED_TOKEN["sid"]:
        ...
        forumAdmin: ForumAdmin = db.session.execute(
            select(ForumAdmin).where(
                (ForumAdmin.forum_id == int(post_mapping["forum"]))
                & (ForumAdmin.user_id == g.DECODED_TOKEN["sid"])
            )
        ).scalar_one_or_none()
        if not forumAdmin:
            raise Forbidden(
                "You do not have the rights to alter this post as you are not its author or an admin in its parent forum"
            )
    ...
    try:
        ...
        with RedisInterface.pipeline(transaction=False) as pipe:
            ...
            pipe.xadd(
                "SOFT_DELETIONS", fields={"id": post_id, "table": Post.__tablename__}
            )
            pipe.execute()
    finally:
    ...
    if redirect := "redirect" in request.args:
        try:
            redirectionForum: int = db.session.execute(
                select(Forum.id).where(Forum.id == int(post_mapping["forum"]))
            ).scalar_one()
        except SQLAlchemyError:
            ...  # Fail silently, ain't no way we have done all that and then do a 500 because we didn't get a redirection link >:(
        ...
        jsonify(
            {
                ...
                "redirect": (
                    ...
                    if not redirect
                        ...
                        "FORUMS_BLUEPRINT.get_forum",
                        _external=False,
                        forum_id=redirectionForum,
                    )
                ),
                "post": post_mapping,
            }
...
def vote_post(post_id: int) -> tuple[Response, int]:
    try:
        ...
        if vote != 0 and vote != 1:
            raise BadRequest(
                "Invalid vote value (Should be 0 (downvote) or 1 (upvote))"
            )
    except KeyError:
    ...
    incoming_intent: str = (
        ...
        if vote == 1
        else RedisConfig.RESOURCE_CREATION_PENDING_ALT_FLAG
    )  # Alt flag is special value for downvotes. We need this here because a downvote is still resource creation, but different than an upvote obviously
    delta: int = 1
    cache_key: str = f"{Post.__tablename__}:{post_id}"
    ...
    post_mapping, latest_intent, queued_deletion = resource_cache_precheck(
        RedisInterface,
        post_id,
        cache_key,
        f"delete:{cache_key}",
        flag_key,
        lock_key,
        conflicting_intent=incoming_intent,
    )

    previous_vote: bool = (
        True
        if latest_intent == RedisConfig.RESOURCE_CREATION_PENDING_FLAG
        else (
            False
            if latest_intent == RedisConfig.RESOURCE_CREATION_PENDING_ALT_FLAG
            else None
        )
    )
    ...
    try:
        if not (post_mapping or previous_vote):
            ...
            joined_result: Row = db.session.execute(
                select(Post, PostVote.vote)
                .outerjoin(
                    PostVote,
                    (PostVote.post_id == post_id)
                    & (PostVote.voter_id == g.DECODED_TOKEN["sid"]),
                )
                .where(
                    (Post.id == post_id)
                    & (Post.deleted.is_(False))
    ...
    if not post_mapping:
        ...
        raise NotFound(f"No post with ID {post_id} exists")
    if (
        previous_vote and incoming_intent == RedisConfig.RESOURCE_CREATION_PENDING_FLAG
    ) or (
        previous_vote is False
        and incoming_intent == RedisConfig.RESOURCE_CREATION_PENDING_ALT_FLAG
    ):
        RedisInterface.delete(lock_key)
    ...
    if previous_vote is not None:
        ...
        try:
            ...
            db.session.rollback()
            raise InternalServerError(
                "Failed to case vote on this post, please try again sometime later"
            )
        except Exception as e:
            RedisInterface.delete(lock_key)
            raise e
    ...
    if not vote:
        delta *= -1  # Negative vote for downvote
    try:
        ...
        update_global_counter(
            ...
            table=User.__tablename__,
            column="aura",
            identifier=post_mapping.get("author_id"),
        )
        ...
        with RedisInterface.pipeline() as pipe:
            ...
            pipe.xadd(
                "WEAK_INSERTIONS",
                {
                    "voter_id": g.DECODED_TOKEN["sid"],
                    "post_id": post_id,
                    "vote": vote,
                    "table": PostVote.__tablename__,
                },
            )
            pipe.execute()
    finally:
...
def unvote_post(post_id: int) -> tuple[Response, int]:
    ...
    delta: int = (
        ...
        if latest_intent == RedisConfig.RESOURCE_CREATION_PENDING_FLAG
            ...
            -1 if latest_intent == RedisConfig.RESOURCE_CREATION_PENDING_ALT_FLAG else 0
        )
    )
    ...
    try:
        if not (post_mapping or latest_intent):
            ...
            previous_vote: bool = db.session.execute(
                select(PostVote.vote).where(
                    (PostVote.voter_id == g.DECODED_TOKEN["sid"])
                    & (PostVote.post_id == post_id)
                )
    ...
    if (
        not delta
    ):  # Neither cache nor DB was able to prove that the user has previously voted on this post, hence this request is invalid. Begone >:3
        RedisInterface.delete(lock_key)
    ...
    response_body: dict = {
        ...
        "previous_vote": "upvote" if delta == -1 else "downvote",
    }
    if queued_deletion:
        response_body["deletion_notice"] = (
            f"Post {post_id} will be deleted soon, and you may not be able to cast votes on this post henceforth"
        )
...
def save_post(post_id: int) -> tuple[Response, int]:
    ...
    post_mapping, latest_intent, deletion_intent = resource_cache_precheck(
        RedisInterface,
        post_id,
        cache_key,
        f"delete:{cache_key}",
    ...
    isSaved: bool = (
        False if latest_intent == RedisConfig.RESOURCE_DELETION_PENDING_FLAG else True
    )
    try:
        if not (post_mapping or latest_intent):
            # Nothing known at this point, query both Post and PostSave
            joined_result: Row = db.session.execute(
                select(Post, PostSave)
                .outerjoin(
                    PostSave,
                    (PostSave.post_id == post_id)
                    & (PostSave.user_id == g.DECODED_TOKEN["sid"]),
                )
                .where(
                    (Post.id == post_id)
                    & (Post.deleted.is_(False))
...
def unsave_post(post_id: int) -> tuple[Response, int]:
    ...
    isSaved: bool = (
        False if latest_intent == RedisConfig.RESOURCE_CREATION_PENDING_FLAG else True
    )
    try:
        if not (post_mapping or latest_intent):
            # Nothing known at this point, query both Post and PostSave
    ...
    if deletion_intent:
        response_body["deletion_notice"] = (
            f"Post {post_id} will be deleted soon, and you may not be able to save this post henceforth"
        )
    return jsonify(response_body), 202
...
def report_post(post_id: int) -> tuple[Response, int]:
    ...
    try:
        ...
        if not (report_desc and report_tag):
            raise BadRequest(
                "Report request must contain description and a valid report reason"
            )
    ...
    cache_key: str = f"{Post.__tablename__}:{post_id}"
    flag_key: str = (
        f'{PostReport.__tablename__}:{g.DECODED_TOKEN["sid"]}:{post_id}:{report_tag}'  # It is important to include report tag here to differentiate between same reports with different tags
    )
    lock_key: str = f"lock:{flag_key}"

    post_mapping, latest_intent = posts_cache_precheck(
        client=RedisInterface,
        post_id=post_id,
        post_cache_key=cache_key,
        post_deletion_intent_flag=f"delete:{cache_key}",
        action_flag=flag_key,
        lock_name=lock_key,
    )
    ...
    try:
        if not (post_mapping or latest_intent):
            ...
            _res = db.session.execute(
                select(Post.id, PostReport)
                .outerjoin(
                    PostReport,
                    (PostReport.user_id == g.DECODED_TOKEN["sid"])
                    & (PostReport.post_id == post_id)
                    & (PostReport.report_tag == report_tag),
                )
                .where(
                    (Post.id == post_id)
                    & (Post.deleted.is_(False) & (Post.rtbf_hidden.isnot(True)))
            ...
            priorReport = db.session.execute(
    ...
    try:
        ...
        with RedisInterface.pipeline() as pipe:
            pipe.set(
                ...
                RedisConfig.RESOURCE_CREATION_PENDING_FLAG,
                ex=RedisConfig.TTL_STRONGEST,
            )  # Flag value is irrelevant here since existence alone is used in cache prechecks, included only for consistency
            pipe.xadd(
                "WEAK_INSERTIONS",
                {
                    "user_id": g.DECODED_TOKEN["sid"],
                    ...
                    "table": PostReport.__tablename__,
                },
            )
            pipe.execute()
    finally:
...
def get_post_comments(post_id: int) -> tuple[Response, int]:
    ...
    post_mapping: dict[str, Any] = resource_existence_cache_precheck(
        client=RedisInterface,
        identifier=post_id,
        resource_name=User.__tablename__,
        cache_key=cache_key,
    )
    if not post_mapping:
    ...
    comments, end, next_cursor = fetch_group_resources(
    ...
    if comments and all(comments):
        counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
            client=RedisInterface,
            hashmaps=[f"{Comment.__tablename__}:{attr}" for attr in counter_attrs],
        ...
        comments: list[dict[str, Any]] = [
            pyserialize(comment, Comment.deserialize_mapping()) for comment in comments
        ]
    ...
    try:
        ...
        next_comments: list[tuple[Comment, str]] = db.session.execute(
    ...
    jsonified_comments: list[dict[str, Any]] = [
        comment_data[0].__json_like__() | {"username": comment_data[1]}
        for comment_data in next_comments
    ]
    # Cache grouped resources with updated counters
    cache_grouped_resource(
        RedisInterface,
        group_key=pagination_cache_key,
        resource_type=Comment.__tablename__,
        resources={
            jsonified_comment["id"]: rediserialize(jsonified_comment)
            for jsonified_comment in jsonified_comments
        },
    ...
    counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
        client=RedisInterface,
        hashmaps=[f"{Comment.__tablename__}:{attr}" for attr in counter_attrs],
        identifiers=[comment["id"] for comment in jsonified_comments],
    )
    for idx, counters in enumerate(counters_mapping.values()):
        for comment_idx, counter in enumerate(counters):
            if counter is not None:
    ...
    return (
        jsonify({"comments": jsonified_comments, "cursor": next_cursor, "end": end}),
        200,
    )
