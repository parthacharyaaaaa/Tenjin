...
def register() -> tuple[Response, int]:
    if not (
        g.REQUEST_JSON.get("username")
        and g.REQUEST_JSON.get("password")
        and g.REQUEST_JSON.get("email")
    ):
        raise BadRequest("Response requires username, password, and email")
    op, USER_DETAILS = processUserInfo(
        username=g.REQUEST_JSON["username"],
    ...
    if existingUsers:
        ...
        if (
            not (existingUsers[0].deleted and existingUsers[-1].deleted)
            or existingUsers[0].time_deleted + current_app["ACCOUNT_RECOVERY_PERIOD"]
            >= current_date
            or existingUsers[-1].time_deleted + current_app["ACCOUNT_RECOVERY_PERIOD"]
            >= current_date
        ):
            conflict = Conflict(
    ...
    try:
        ...
        db.session.rollback()
        conflict = Conflict(
            "An account with this username/email was made at the same time as your request. Tough luck :("
        )
        conflict.__setattr__("kwargs", response_kwargs)
        raise conflict
    except:
...
def delete_user(user_id: int) -> tuple[Response, int]:
    if not (g.REQUEST_JSON.get("username") and g.REQUEST_JSON.get("password")):
        raise BadRequest("Requires username and password to be provided")

    OP, USER_DETAILS = processUserInfo(
        username=g.REQUEST_JSON["username"], password=g.REQUEST_JSON["password"]
    )
    if not OP:
        raise BadRequest(USER_DETAILS.get("error"))
    target_user: User = db.session.execute(
        select(User).where(
            (User.username == USER_DETAILS["username"]) & (User.deleted.is_(False))
        )
    ).scalar_one_or_none()
    if not target_user:
    ...
    if not verify_password(
        g.REQUEST_JSON["password"], target_user.pw_hash, target_user.pw_salt
    ):
    ...
    if user_mapping and RedisConfig.NF_SENTINEL_KEY in user_mapping:
        ...
        raise NotFound(
            "No user with this ID found. Perhaps you already deleted this account?"
        )
    if lock or latest_intent == RedisConfig.RESOURCE_DELETION_PENDING_FLAG:
        raise Conflict(
            "Another request for this account's deletion is already underway"
        )
    ...
    if not lock_set:
        raise Conflict(
            "Failed to perform this action, another request for the same action is being processed"
        )
    ...
    try:
        ...
        with RedisInterface.pipeline(transaction=False) as pipe:
            ...
            pipe.xadd(
                "USER_ACTIVITY_DELETIONS",
                {
                    "user_id": target_user.id,
                    "rtbf": int(target_user.rtbf),
                    "table": User.__tablename__,
                },
            )
            pipe.hset(
                cache_key, {RedisConfig.NF_SENTINEL_KEY: RedisConfig.NF_SENTINEL_VALUE}
            )
        ...
        raise InternalServerError(
            "Failed to perform account deletion, please try again. If the issue persists, please raise a ticket"
        )
    finally:
        RedisInterface.delete(lock_key)
    requests.delete(
        url=f"{current_app.config['AUTH_SERVER_URL']}/auth/tokens",
        json={"sub": target_user.username, "sid": target_user.id},
    )
    return (
        jsonify(
            {
                ...
                "username": target_user.username,
                "time_deleted": target_user.time_deleted,
            }
        ),
        200,
    )
...
def recover_user() -> Response:
    ...
    deadAccount = db.session.execute(
        select(User)
        .where(
            (
                User.email == USER_DETAILS.get("email")
                if isEmail
                else User.username == USER_DETAILS["username"]
            )
        )
        .with_for_update()
    ).scalar_one_or_none()
    ...
    if not deadAccount:
        # Never existed
        nf = NotFound(
            f"No deleted account with this {'email' if isEmail else 'username'} exists."
        )
        nf.__setattr__(
            "kwargs",
            {
                "info": f"If you had believe that this account is still in the recovery period of {current_app['ACCOUNT_RECOVERY_PERIOD']} days, contact support",
    ...
    if (
        deadAccount.time_deleted or datetime(2000, 1, 1, 0, 0, 0, 0)
    ) + current_app.config["ACCOUNT_RECOVERY_PERIOD"] <= datetime.now():
        # Tough luck, its already past recovery period >:(
        cnf = Conflict(
            "Account recovery period for this account has already expired, and its deletion is due soon. Please create a new account"
        )
        raise cnf

    # Match password hashes
    if not verify_password(
        USER_DETAILS["password"], deadAccount.pw_hash, deadAccount.pw_salt
    ):
        unauth = Unauthorized("Incorrect password")
        unauth.__setattr__(
            "kwargs",
            {
                "info": "If you have forgotten your password and want to recover your account, please perform the deleted account password recovery phase",
    ...
    if (
        not deadAccount.deleted
        or latest_intent == RedisConfig.RESOURCE_DELETION_PENDING_FLAG
    ):  # If both cache and DB fail to verify the prior deletion of this account, raise Conflict
    ...
    try:
        ...
        with RedisInterface.pipeline(transaction=False) as pipe:
            ...
            pipe.xadd(
                "USER_ACTIVITY_RECOVERY",
                {
                    "user_id": deadAccount.id,
                    "rtbf": int(deadAccount.rtbf),
                    "table": User.__tablename__,
                },
            )
        ...
        enqueueEmail(
            ...
            user_id=deadAccount.id,
            time_restored=datetime.now().isoformat(),
        )
    except:
        raise InternalServerError(
            "Failed to recover your account. If this issue persists, please raise a user ticket immediately"
        )
    finally:
        RedisInterface.delete(lock_key)
...
def recover_password() -> Response:
    ...
    OP, USER_DETAIL = processUserInfo(
        email=g.REQUEST_JSON.get("identity") if isEmail else None,
    ...
    recoveryAccount = db.session.execute(
        select(User).where(
            User.email == USER_DETAIL.get("email")
            if isEmail
            else User.username == USER_DETAIL["username"]
        )
    ).scalar_one_or_none()

    if not recoveryAccount:
        raise NotFound(
            f"No account with this {'email' if isEmail else 'username'} exists"
        )
    ...
    try:
        db.session.execute(
            delete(PasswordRecoveryToken).where(
                PasswordRecoveryToken.user_id == recoveryAccount.id
            )
        )
        db.session.execute(
            insert(PasswordRecoveryToken).values(
                user_id=recoveryAccount.id,
        ...
        raise InternalServerError(
            "There seems to be an issue with our password recovery service"
        )
    enqueueEmail(
        RedisInterface,
        ...
        password_recovery_link=url_for(
            "templates.recover_password", _external=True, digest=temp_url
        ),
    )
...
def update_password(temp_url: str) -> Response:
    ...
    dbToken: PasswordRecoveryToken = db.session.execute(
        select(PasswordRecoveryToken).where(
            PasswordRecoveryToken.url_hash == hashedToken
        )
    ).scalar_one_or_none()
    if not dbToken:
    ...
    try:
        ...
        db.session.execute(
            delete(PasswordRecoveryToken).where(
                PasswordRecoveryToken.user_id == dbToken.user_id
            )
        )
        db.session.commit()
...
def get_user(user_id: int) -> tuple[Response, int]:
    ...
    user_mapping: dict[str, str | int] = consult_cache(
        RedisInterface,
        cacheKey,
        RedisConfig.TTL_CAP,
    ...
    if user_mapping:
        ...
        user_mapping: dict[str, Any] = pyserialize(
            user_mapping, deserialize_mapping=User.deserialize_mapping()
        )

        # Update with global counters
        counter_mapping: dict[str, Sequence[int]] = fetch_global_counters(
            RedisInterface,
            [f"{User.__tablename__}:{attr}" for attr in counter_attrs],
            identifiers=[user_id],
        )
...
def get_user_posts(user_id: int) -> tuple[Response, int]:
    ...
    user_mapping: dict[str, Any] = resource_existence_cache_precheck(
    ...
    jsonified_posts: list[dict[str, Any]] = [
        post.__json_like__() | {"username": user_mapping["username"]}
...
def get_user_forums(user_id: int) -> tuple[Response, int]:
    ...
    forums, end, next_cursor = fetch_group_resources(
        RedisInterface, group_key=pagination_cache_key
    )
    ...
    if forums and all(forums):
        counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
            client=RedisInterface,
            hashmaps=[f"{Forum.__tablename__}:{attr}" for attr in counter_attrs],
            identifiers=[forum["id"] for forum in forums],
        )
    ...
    try:
        ...
        joined_forum_res: list[tuple[Forum, datetime]] = db.session.execute(
            select(Forum, ForumSubscription.time_subscribed)
            .join(
                ForumSubscription,
                (Forum.id == ForumSubscription.forum_id)
    ...
    jsonified_forums: list[dict[str, Any]] = [
        res[0].__json_like__() | {"time_subscribed": res[1]} for res in joined_forum_res
    ]
    ...
    cache_grouped_resource(
        ...
        resource_type=Post.__tablename__,
        resources={
            jsonified_forum["id"]: rediserialize(jsonified_forum)
            for jsonified_forum in jsonified_forums
        },
        weak_ttl=RedisConfig.TTL_WEAK,
        strong_ttl=RedisConfig.TTL_STRONG,
        cursor=next_cursor,
    ...
    counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
        client=RedisInterface,
        hashmaps=[f"{Forum.__tablename__}:{attr}" for attr in counter_attrs],
        identifiers=[forum["id"] for forum in jsonified_forums],
    )
    for idx, counters in enumerate(counters_mapping.values()):
        for forum_idx, counter in enumerate(counters):
            if counter is not None:
...
def get_user_animes(user_id: int) -> tuple[Response, int]:
    ...
    animes, end, next_cursor = fetch_group_resources(
        RedisInterface, group_key=pagination_cache_key
    )
    ...
    if animes and all(animes):
        counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
            client=RedisInterface,
            hashmaps=[f"{Anime.__tablename__}:{attr}" for attr in counter_attrs],
            identifiers=[anime["id"] for anime in animes],
        )
        for idx, counters in enumerate(counters_mapping.values()):
    ...
    # Cache miss
    try:
        next_anime_res: list[tuple[Anime, datetime]] = db.session.execute(
            select(Anime, AnimeSubscription.time_subscribed)
    ...
    jsonified_animes: list[dict[str, Any]] = [
        row[0].__json_like__() | {"time_subscribed": row[1].isoformat()}
        for row in next_anime_res
    ]
    # Cache grouped resources
    cache_grouped_resource(
        RedisInterface,
        group_key=pagination_cache_key,
        resource_type=Anime.__tablename__,
        resources={
            jsonified_anime["id"]: jsonified_anime
            for jsonified_anime in jsonified_animes
        },
        weak_ttl=RedisConfig.TTL_WEAK,
        strong_ttl=RedisConfig.TTL_STRONG,
        cursor=next_cursor,
    ...
    counters_mapping: dict[str, Sequence[int | None]] = fetch_global_counters(
        client=RedisInterface,
        hashmaps=[f"{Anime.__tablename__}:{attr}" for attr in counter_attrs],
        identifiers=[anime["id"] for anime in jsonified_animes],
    )
    for idx, counters in enumerate(counters_mapping.values()):
        for anime_idx, counter in enumerate(counters):
            if counter is not None:
...
def login() -> Response:
    ...
    if "@" in identity:
        ...
        if not 5 < len(identity) <= 64:
            raise BadRequest(
                "Provided username must be between 5 and 64 characters long"
            )
    ...
    try:
        user: User = db.session.execute(
            select(User)
            .where(
                (User.email == identity if isEmail else User.username == identity)
    ...
    if not user:
        raise NotFound(
            f"No user with {'email' if isEmail else 'username'} {identity} could be found"
        )
    ...
    if deletion_intent == RedisConfig.RESOURCE_DELETION_PENDING_FLAG:
        gone: Gone = Gone(
            "This account has been deleted. If you wish to undo this, please visit account recovery"
        )
        gone.kwargs = {
            "links": {
                "account recovery": {"_href": url_for(".recover_user", _external=True)}
            }
        }
        raise gone
    ...
    try:
        db.session.execute(
            update(User).where(User.id == user.id).values(last_login=epoch)
        )
        db.session.commit()
    except:
        db.session.rollback()
...
def enable_rtbf(user_id: int) -> tuple[Response, int]:
    ...
    user_rtbf: bool = db.session.execute(
        select(User.rtbf).where(User.id == user_id).with_for_update(nowait=True)
    ).scalar_one_or_none()
    ...
    try:
        db.session.execute(update(User).where(User.id == user_id).values(rtbf=True))
        db.session.commit()
    except SQLAlchemyError:
        raise InternalServerError(
            "Failed to update RTBF settings at the moment, please try again later"
        )
...
def disable_rtbf(user_id: int) -> tuple[Response, int]:
    ...
    try:
        db.session.execute(update(User).where(User.id == user_id).values(rtbf=False))
        db.session.commit()
    except SQLAlchemyError:
        raise InternalServerError(
            "Failed to updatwe RTBF settings at the moment, please try again later"
        )
