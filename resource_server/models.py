...
class ForumSubscription(db.Model):
    ...
    user_id: int = db.Column(
        db.BigInteger, db.ForeignKey("users.id", ondelete="CASCADE"), primary_key=True
    )
    forum_id: int = db.Column(
        db.Integer, db.ForeignKey("forums.id", ondelete="CASCADE"), primary_key=True
    )
    time_subscribed: datetime = db.Column(
...
class AnimeSubscription(db.Model):
    ...
    anime_id: int = db.Column(
        db.Integer, db.ForeignKey("animes.id", ondelete="CASCADE"), primary_key=True
    )
    time_subscribed: datetime = db.Column(
        db.TIMESTAMP, nullable=False, server_default=text("CURRENT_TIMESTAMP")
    )
...
class PostVote(db.Model):
    ...
    voter_id: int = db.Column(
        db.BigInteger, db.ForeignKey("users.id"), primary_key=True
    )
    post_id: int = db.Column(
        db.BigInteger, db.ForeignKey("posts.id", ondelete="CASCADE"), primary_key=True
    )
...
class User(db.Model):
    ...
    time_joined: datetime = db.Column(
...
class UserTicket(db.Model):
    ...
    time_raised: datetime = db.Column(
        TIMESTAMP, nullable=False, server_default=text("CURRENT_TIMESTAMP")
    )
...
class Forum(db.Model):
    ...
    anime: int = db.Column(
        INTEGER, db.ForeignKey("animes.id"), index=True, nullable=True
    )
...
class ForumRules(db.Model):
    ...
    # Identification
    forum_id: int = db.Column(
        INTEGER, db.ForeignKey("forums.id", ondelete="CASCADE"), nullable=False
    )

    # Data
    rule_number: int = db.Column(
        SMALLINT, nullable=False, unique=True, autoincrement=True
    )
    title: str = db.Column(VARCHAR(32), nullable=False)
    body: str = db.Column(
        VARCHAR(128), server_default="No additional description provided for this rule."
    )
...
class Post(db.Model):
    ...
    author_id: int = db.Column(
        BIGINT, db.ForeignKey("users.id"), nullable=False, index=True
    )
    forum_id: int = db.Column(
        INTEGER, db.ForeignKey("forums.id", ondelete="CASCADE"), nullable=False
    )
