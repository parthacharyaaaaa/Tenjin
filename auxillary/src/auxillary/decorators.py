...
def enforce_json(endpoint):
    ...
    def decorated(*args, **kwargs):
        if not request.mimetype or request.mimetype.split("/")[-1] != "json":
            raise BadRequest(
                "Invalid mimetype in request headers (Content-Type), must be */json"
            )
        ...
        if not g.REQUEST_JSON:
            ...
            badReq.__setattr__(
                "details",
                "Please ensure that request body is formatted as JSON and does not contain any syntax errors or invalid data types",
            )
            raise badReq
...
def private(endpoint):
    ...
    def decorated(*args, **kwargs):
        try:
            ...
            # HTTP check
            if (
                not request.headers["PRIVATE-API-KEY"]
                in current_app.config["PRIVATE_COMM_KEYS"]
            ):
