...
class FlaskConfig:
    try:
        ...
        SUSPICIOUS_LOOKBACK_TIME: timedelta = timedelta(
            days=int(os.environ["SUSPICIOUS_LOOKBACK_TIME"])
        )
        ...
        ANNOUNCEMENT_DURATION: int = int(
            os.environ.get("ANNOUNCEMENT_DURATION", 900)
        )  # Duration in seconds
        ADMIN_SESSION_DURATION: int = int(
            os.environ["ADMIN_SESSION_DURATION"]
        )  # Duration in seconds
        MAX_VALID_KEYS: int = int(os.environ["MAX_VALID_KEYS"])
        KEY_ROTATION_COOLDOWN: int = int(
            os.environ.get("KEY_ROTATION_COOLDOWN", 60 * 60 * 24)
        )  # Default to 1 day in seconds

    except KeyError as e:
        raise ValueError(
            f"FAILED TO SETUP CONFIGURATIONS FOR FLASK AUTH APPLICATION AS ENVIRONMENT VARIABLES WERE NOT FOUND (SEE: class Flask_Config at '{__file__}')"
        )
    except TypeError as e:
        raise TypeError(
            f"FAILURE IN CONFIGURING ENVIRONMENT VARIABLE(S) OF TYPE: INT (SEE: class Flask_Config at '{__file__}')"
        )
