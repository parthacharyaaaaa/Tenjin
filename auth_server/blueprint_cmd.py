...
def admin_login() -> tuple[Response, int]:
    ...
    encodedSessionToken: str = base64.urlsafe_b64encode(
        ujson.dumps(sessionMapping).encode()
    ).decode()
    return (
        jsonify(
            {"session_token": encodedSessionToken, "revival_digest": revivalDigest}
        ),
        200,
    )
...
def admin_refresh() -> tuple[Response, int]:
    ...
    encodedSessionToken: str = base64.urlsafe_b64encode(
        ujson.dumps(newSessionMapping).encode()
    ).decode()
    return (
        jsonify(
            {"session_token": encodedSessionToken, "revival_digest": revivalDigest}
...
def get_key(kid: str) -> tuple[Response, int]:
    try:
        key = db.session.execute(
            select(KeyData).where(KeyData.kid == kid)
        ).scalar_one_or_none()
    except SQLAlchemyError:
        genericDBFetchException()
...
def invalidate_key(kid: str) -> tuple[Response, int]:
    ...
    try:
        ...
        if not target_key.rotated_out_at:
            ...
            report_suspicious_activity(
                g.SESSION_TOKEN["admin_id"], f"Invaldiation attempt on active key {kid}"
            )
            raise Conflict(
    ...
    try:
        ...
        validInactiveKeys: list[str] = (
            db.session.execute(
                select(KeyData.kid)
                .where(
                    (KeyData.expired_at == None) & (KeyData.rotated_out_at.isnot(None))
                )
        ...
        activeKey: KeyData = db.session.execute(
        ...
        activeKeyMapping: dict[str, Any] = {
            ...
            "use": "sig",
            "kid": activeKey.kid,
            "x": to_base64url(int(vk.pubkey.point.x())),
            "y": to_base64url(int(vk.pubkey.point.y())),
        }
...
def rotate_keys() -> tuple[Response, int]:
    ...
    if cooldown_flag and g.SESSION_TOKEN["role"] == "staff":
        report_suspicious_activity(
            ...
            desc="Attempt to perform key rotation during cooldown",
        )
        raise Conflict(
            "The server is currently undergoing a key rotation cooldown, and will not accept rotation requets. Repeated attempt will lead to account lock"
        )
    ...
    try:
        ...
        db.session.execute(
            insert(KeyData).values(
                ...
                private_pem=signingKey.to_pem(),
                public_pem=verificationKey.to_pem(),
            )
        )
